...
void filtragepriorite(tache tab[], int n) {
    char priorite;
    int i;
    printf("Entrer une prioritÈ : 'H' pour high, 'L' pour low : ");
    scanf(" %c", &priorite);  // Utiliser %c pour un char
    switch (priorite) {
        case 'L':
            for (i = 0; i < n; i++) {
                if (strcmp(tab[i].priorite, "low") == 0) {  // pour comparer la chaÓne de caractËres stockÈe dans tab[i].priorite avec la chaÓne "low".
                    printf("Titre : %s\n", tab[i].name);
                    printf("Description : %s\n", tab[i].description);
                    printf("Date : %d/%d/%d\n", tab[i].date.jour, tab[i].date.mois, tab[i].date.annee);
                    printf("PrioritÈ : %s\n", tab[i].priorite);
        ...
        case 'H':
            for (i = 0; i < n; i++) {
                if (strcmp(tab[i].priorite, "high") == 0) {  // pour comparer la chaÓne de caractËres stockÈe dans tab[i].priorite avec la chaÓne "high".
                    printf("Titre : %s\n", tab[i].name);
                    printf("Description : %s\n", tab[i].description);
                    printf("Date : %d/%d/%d\n", tab[i].date.jour, tab[i].date.mois, tab[i].date.annee);
                    printf("PrioritÈ : %s\n", tab[i].priorite);
        ...
        default:
            printf("PrioritÈ non reconnue. Veuillez entrer 'H' ou 'L'.\n");
    }
